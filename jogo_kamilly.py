...
# --- 4. ESTILIZAÇÃO CSS (SUPER MOBILE) ---
st.markdown("""
    <style>
    ...
    /* FORÇA 3 COLUNAS LADO A LADO NO ANDROID */
    ...
    h1 { color: #ffd700; text-align: center; font-size: 20px; text-shadow: 0 0 10px #ffd700; }
...
if opcao == "🎰 Jackpot":
    ...
    for r in range(3):
        ...
        for c in range(3):
            ...
            # Pega dados com fallback total para evitar erros
            dados = familia.get(nome_p, ["", "❓"])
            foto, emoji = dados, dados[1]

            # Tentativa de carregar imagem simplificada (Sem os.path que causa erro)
            try:
                cols[c].image(foto, use_container_width=True)
            except:
                cols[c].markdown(f"""
                    <div style='height:80px; background:#222; border-radius:10px; display:flex; 
